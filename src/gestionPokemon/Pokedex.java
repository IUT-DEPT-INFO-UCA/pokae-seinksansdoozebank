...
public class Pokedex {
        ...
        try (Scanner scannerCSV = new Scanner(fichierCSV)) {
            ...
            while (scannerCSV.hasNext()) {
                if (Integer.parseInt(scannerCSV.next()) == id) {
                    ...
                    // ca c'est dramatique pcq mewtwo doit pas evoluer et pourtant la il peut
                    // evoluer en mew
     ...
     * JSONObject jsonCapacite =
     * getJSONfromURL("https://pokeapi.co/api/v2/move/"+i);
     ...
     * }
     * 
     * if ((((JSONObject)jsonCapacite.get("damage_class")).get("name").toString()).
     * equals("physical")){
     ...
     * }
     * JSONObject
     * jsonNomType=getJSONfromURL(((JSONObject)jsonCapacite.get("type")).get("url").
     * toString());
     * assert jsonNomType != null;
     * capacite.type=new
     * Type((((JSONObject)(((JSONArray)jsonNomType.get("names")).get(3))).get("name"
     * ).toString()));
